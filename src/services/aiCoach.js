...
const openai = openaiConfig.apiKey
  ...
  : null;
const MODEL = openaiConfig.model || 'gpt-4-turbo-preview';

// ─── Personality Type Descriptions ───────────────────────────────
// Used to customize tone and framing for each user type.
const PERSONALITY_PROFILES = {
  'The Seeker': {
    tone: 'deep, reflective, intellectually engaging',
    approach: 'Ask probing questions. Reference the hidden meanings in teachings. Speak to their longing for truth and understanding.',
    strengths: 'depth, sincerity, spiritual hunger',
    challenges: 'overthinking, isolation, idealism',
  },
  'The Nurturer': {
    tone: 'warm, emotionally attuned, gentle',
    approach: 'Lead with empathy and validation. Connect teachings to relationships and family. Speak to their heart first.',
    strengths: 'compassion, loyalty, emotional intelligence',
    challenges: 'boundaries, self-neglect, people-pleasing',
  },
  'The Builder': {
    tone: 'direct, practical, action-oriented',
    approach: 'Give concrete steps and practical guidance. Connect teachings to real-world outcomes. Keep it actionable.',
    strengths: 'determination, leadership, reliability',
    challenges: 'impatience, control, work-life balance',
  },
  'The Peacemaker': {
    tone: 'calm, measured, harmonizing',
    approach: 'Avoid confrontation in framing. Emphasize unity, shalom, and the middle path. Help them find their voice gently.',
    strengths: 'harmony, acceptance, diplomacy',
    challenges: 'indecision, self-abandonment, conflict avoidance',
  },
};
...
export async function buildPersonalizedPrompt(user) {
  ...
  // 4. Build user-specific personality section
  const profile        = PERSONALITY_PROFILES[user?.personalityType] || null;
...
Human Design:     ${user.humanDesignType || 'not yet determined'}

Tone to use:      ${profile.tone}
Approach:         ${profile.approach}
Affirm these strengths: ${profile.strengths}
Gently address:   ${profile.challenges}
`
    : '';
